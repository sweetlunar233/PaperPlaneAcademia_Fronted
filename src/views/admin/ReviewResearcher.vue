  ...
  export default {
    ...
    methods: {
      ...
      async fetchApplications() {
        try {
          const response = await axios.get(`/applications/`);  // 假设API路径是 /applications/
      ...
      async approveApplication(applicationId) {
        try {
          await axios.post(`/applications/${applicationId}/approve`);  // 使用applicationId
          alert("科研人员申请已通过");
      ...
      async submitRejection() {
        try {
          await axios.post(`/applications/${this.selectedApplicationId}/reject/`, { // 使用选中的applicationId
